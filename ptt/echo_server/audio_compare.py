...
class AudioSimilar:
    def __init__(self, sample_time=1000, span=100, step=1, min_overlap=20, threshold=0.85):
    ...
    def calculate_fingerprints(self, filename):
        if os.path.exists(filename + '.fpcalc'):
            ...
            fpcalc_out = str(subprocess.check_output(['fpcalc', '-raw', '-length', str(
                self.sample_time), filename])).strip().replace('\\n', '').replace("'", "")

        fingerprint_index = fpcalc_out.find('FINGERPRINT=') + 12
        # convert fingerprint to list of integers
        fingerprints = list(
            map(int, fpcalc_out[fingerprint_index:].split(',')))
    ...
    def get_max_corr(self, corr, source, target):
        ...
        return {"max_corr_offset": max_corr_offset, "max_corr": corr[max_corr_index]}
    ...
    def correlate(self, source, target):
        ...
        corr = self.compare(fingerprint_source,
                            fingerprint_target, self.span, self.step)
        corr_data = self.get_max_corr(corr, source, target)
        return corr_data

    def calc(self, src, dst):
        if not src or not dst:
            raise Exception("Source or Target files not specified.")

        corr_data = self.correlate(src, dst)
        return corr_data
